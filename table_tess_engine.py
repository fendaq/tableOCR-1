...
#tesseract网络加载
tesseractport.tess_init_chinese()
...
def singleGrid(gridImgs, xcoords, resultPath):
    ...
    if len(gridImgs) != len(xcoords):
        print 'Error! length gridImg != length xcoords'
...
def ctpnGrid(gridImgs, xcoords, reultPath):
    ctpngrids = []
    if len(gridImgs)!=len(xcoords):
        print 'Error! length gridImg != length xcoords: warning'
    for lineIndex, gridImg in enumerate(gridImgs):
        xcoord = xcoords[lineIndex]
        if xcoord == None:
            xcoord = []
    ...
    for img_name in os.listdir(image_dir):
        ...
        for line_index, line in enumerate(ctpngrids):
            for grid_index, grid in enumerate(line):
                ...
                height = grid.shape[0]
                width = grid.shape[1]
                counter = np.zeros([1, width], dtype = np.int)
                ...
                for i in range(width):
                    ...
                    counter[0][i] = number
                # cv2.imshow('grid', grid)
                # cv2.imshow('proj', counter)
                # cv2.waitKey(0)
                former_num = 0
                begin_point = 0
                end_point = -1
                for i in range(width):
                    if counter[0][i] - former_num > 5 and begin_point == 0:
                        begin_point = i
                    if counter[0][i] - former_num < -5:
                        end_point = i
                '''

                name = os.path.join(tmp_dir, 'temp.jpg')
                cv2.imwrite(name, subimg)

                cv2.imwrite(os.path.join(tmp_dir, img_name[:-4] , str(line_index)+  '_' + str(grid_index) + '.jpg'), subimg)
                chinese_seq = tesseractport.tess_recognition_from_file_chinese(name)
                ...
                count_of_number = 0
                for character in chinese_seq:
                    if character.isdigit():
                        count_of_number += 1

                if count_of_number > 3:
                    tess_num_text = tesseractport.tess_recognition_from_file_english(name)
                    revised_number_seq = ''
